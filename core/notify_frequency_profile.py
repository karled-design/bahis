...
def _apply_level_values(level: _NotifyLevel, *, persist_risk: bool = False) -> OperatorRiskSettings:
    risk_settings = update_operator_risk_settings(dict(level["risk"]), persist=persist_risk)
    update_scan_league_settings({"leagues_per_scan": int(level["leagues_per_scan"])})
    return risk_settings


def apply_notify_frequency_level(
    level_id: str, *, persist: bool = True, persist_risk: bool = False
) -> dict[str, Any]:
    ...
    _apply_level_values(level, persist_risk=persist_risk)
