...
_SUPPORTED_API_MARKETS = frozenset({"h2h", "totals"})
...
def _fetch_odds_api_json(url: str) -> tuple[list[Any] | None, dict[str, Any]]:
    ...
    if isinstance(payload, dict):
        ...
        if "quota" in lowered or "usage" in lowered or "exceeded" in lowered:
            ...
            meta["error_kind"] = "parse"
        meta["detail"] = str(message or "unexpected JSON object response")
        return None, meta
...
def fetch_settlement_results(
    ...
    if extra_sport_keys:
        merged_keys: list[str] = list(sport_keys)
        for key in extra_sport_keys:
            if key not in merged_keys:
                merged_keys.append(key)
        sport_keys = merged_keys
    if not sport_keys:
