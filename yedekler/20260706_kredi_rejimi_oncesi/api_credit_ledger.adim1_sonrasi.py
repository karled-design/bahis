...
def get_last_remaining() -> float | None:
    with _LOCK:
        return _parse_credit_number(_load_ledger().get("last_remaining"))
...
def build_credit_panel_payload() -> dict[str, Any]:
    """Panel icin ozet: bugun/ay harcanan, kalan, gunluk pay onerisi."""
    with _LOCK:
        ledger = _load_ledger()
        today = _day_key()
        ...
        remaining = _parse_credit_number(ledger.get("last_remaining"))
        used = _parse_credit_number(ledger.get("last_used"))
    ...
    daily_allowance = int(remaining // days_left) if remaining is not None else None

    if updated_at:
        remaining_text = str(int(remaining)) if remaining is not None else "?"
        status_label = f"Bugun {int(round(today_spent))} kredi | Kalan {remaining_text}"
    else:
