...
def build_market_prob_index(matches: list[dict[str, Any]]) -> dict[str, dict[str, float]]:
    ...
    for match in matches:
        ...
        if not match_name or not market:
            continue
        prob = implied_win_probability(float(match.get("sharp_odds", 0.0)))
        if prob is None:
            continue
...
def _evaluate_market_superiority(
    ...
    *,
    ...
    other_probs = [prob for key, prob in markets.items() if key != market]
    if not other_probs:
...
def evaluate_hero_gate(
    ...
    win_probability = implied_win_probability(sharp_odds)
