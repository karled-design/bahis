...
def collect_pending_sport_keys(pending_kupons: list[dict[str, Any]]) -> tuple[str, ...]:
    ...
    for fallback in _DEFAULT_SETTLEMENT_SPORT_KEYS:
...
def _log_settlement_cycle_summary(
    ...
    _emit_settlement_diag(
        ...
        f"ek_ligler={','.join(extra_sport_keys) or 'yok'} | "
...
def run_settlement_pass() -> dict[str, int]:
    ...
    stats = {"pending": 0, "settled": 0, "waiting": 0}
    ...
    try:
        ...
        priority_sport_keys = collect_pending_sport_keys(active_pending)
...
def run_auto_settler_loop(stop_event: threading.Event | None = None) -> None:
    ...
    while not shutdown_event.is_set():
        try:
            ...
            sleep_seconds = _IDLE_CYCLE_SLEEP_SECONDS if stats["pending"] == 0 else _CYCLE_SLEEP_SECONDS
        except Exception as exc:
