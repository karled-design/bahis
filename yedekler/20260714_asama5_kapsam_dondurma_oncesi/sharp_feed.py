...
def _enrich_events_with_side_markets(events: list[Any], sport_key: str) -> None:
    ...
    Uc fren birden: strateji dugmesi (varsayilan kapali) + kickoff penceresi
    (36 saat) + gunluk yan-pazar kredi tavani. Herhangi bir hata yalnizca yan
    pazari atlatir; ana tarama ETKILENMEZ.
    """
    try:
        from core.strategy_settings import is_side_markets_enabled

        if not is_side_markets_enabled():
            return
    except Exception as exc:
