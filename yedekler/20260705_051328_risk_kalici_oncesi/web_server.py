...
def _build_status_payload() -> dict[str, Any]:
    ...
    payload["clv_scorecard"] = _load_clv_scorecard()
    ...
    payload["scan_leagues"] = build_scan_leagues_payload()
...
class _OperatorPanelHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if route == "/api/risk_settings":
            ...
            try:
                values = update_operator_risk_settings(body)
        ...
        if route == "/api/risk_preset":
            ...
            values = apply_risk_preset(preset_id)
            if values is None:
        ...
        if route == "/api/notify_frequency":
            ...
            try:
                payload = apply_notify_frequency_level(level_raw.strip())
                update_sistem_durumu(
                    risk_per_trade=get_operator_risk_per_trade(),
                    ev_threshold=get_action_ev_threshold(),
                )
