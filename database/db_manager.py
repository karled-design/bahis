...
def _migrate_kupon_schema(connection: sqlite3.Connection) -> None:
    ...
    if "alinan_oran" not in columns:
        connection.execute("ALTER TABLE kuponlar ADD COLUMN alinan_oran REAL")
...
def _kupon_row_to_dict(row: sqlite3.Row) -> dict[str, Any]:
    ...
    return {
        ...
        "alinan_oran": (
            float(row["alinan_oran"])
            if "alinan_oran" in keys and row["alinan_oran"] is not None
            else None
        ),
    }
...
def add_kupon(
    ...
    *,
    ...
    alinan_value: float | None = None
    if alinan_oran is not None:
        if (
            isinstance(alinan_oran, bool)
            or not isinstance(alinan_oran, (int, float))
            or not _is_valid_odds(float(alinan_oran))
        ):
            _emit_operator_diag("add_kupon | alinan_oran must be > 1.0 when provided")
            return False
        alinan_value = round(float(alinan_oran), 2)
        ...
        try:
            ...
            connection.execute(
                ...
                INSERT INTO kuponlar (
                    ...
                    durum, eklenme_tarihi, sport_key, event_id, commence_time, alinan_oran
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
...
def set_kupon_alinan_oran(kupon_id: int, alinan_oran: float) -> bool:
    """Bir kupona kullanicinin Nesine'de GERCEKTE girdigi orani yazar.

    Panel 'Oynadim' akisinda ve kupon listesinden elle girisde kullanilir.
    Var olan bir kupon guncellenmezse (id yoksa) False doner.
    """
    if isinstance(kupon_id, bool) or not isinstance(kupon_id, int) or kupon_id <= 0:
        _emit_operator_diag("set_kupon_alinan_oran | kupon_id must be a positive int")
        return False
    if (
        isinstance(alinan_oran, bool)
        or not isinstance(alinan_oran, (int, float))
        ...
        _emit_operator_diag("set_kupon_alinan_oran | alinan_oran must be > 1.0")
        return False
    ...
    with _DB_LOCK:
        ...
        try:
            ...
            cursor = connection.execute(
                ...
                WHERE id = ?
                """,
                (round(float(alinan_oran), 2), kupon_id),
            )
            connection.commit()
            return cursor.rowcount > 0
        except sqlite3.Error as exc:
            _emit_operator_diag(f"set_kupon_alinan_oran failed | {exc}")
            return False
        finally:
            connection.close()
